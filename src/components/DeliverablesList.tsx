  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-8">
        <div className="text-zinc-500 text-xs">Loading deliverables...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {deliverables.map((deliverable) => (
        <div
          key={deliverable.id}
          className="flex gap-3 p-3 bg-black/20 rounded border border-white/10 hover:border-[var(--cyber-primary)]/50 transition-colors"
        >
          {/* Icon */}
          <div className="flex-shrink-0 text-[var(--cyber-primary)]">
            ...
            <div className="flex items-start justify-between gap-2">
              {deliverable.deliverable_type === 'url' && deliverable.path ? (
                <a
                  ...
                  className="font-medium text-sm text-[var(--cyber-primary)] hover:text-[var(--cyber-primary)]/80 hover:underline flex items-center gap-1.5"
                >
                  {deliverable.title}
                  <ExternalLink className="w-3.5 h-3.5" />
                </a>
              ) : (
                <h4 className="font-medium text-sm text-zinc-200">{deliverable.title}</h4>
              )}
              <div className="flex items-center gap-1">
                ...
                {deliverable.deliverable_type === 'file' && deliverable.path?.endsWith('.html') && (
                  <button
                    ...
                    className="flex-shrink-0 p-1.5 hover:bg-white/5 rounded text-[var(--cyber-success)]"
                ...
                {deliverable.path && (
                  <button
                    ...
                    className="flex-shrink-0 p-1.5 hover:bg-white/5 rounded text-[var(--cyber-primary)]"
            ...
            {deliverable.path && (
              deliverable.deliverable_type === 'url' ? (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-2 p-2 bg-black/30 rounded text-[10px] text-[var(--cyber-primary)] hover:text-[var(--cyber-primary)]/80 font-mono break-all block"
                >
                  {deliverable.path}
                </a>
              ) : (
                <div className="mt-2 p-2 bg-black/30 rounded text-[10px] text-zinc-500 font-mono break-all">
                  {deliverable.path}
                </div>
              )
            )}
